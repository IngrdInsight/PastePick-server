...
export async function createToothpaste({
	...
	try {
		...
		// 1. Check if similar toothpaste exists by fuzzy search
		const existing_toothpaste = await fuzzyFindSimilarToothpaste(name);

		if (existing_toothpaste) {
			await dbClient.query("ROLLBACK");
			throw new Error(
				`Similar toothpaste already exists: ${existing_toothpaste.name}`,
			);
		}

		// 2. Process ingredients from text (pass client for transaction)
		const ingredients = await processIngredientsFromText(ingredients_raw, dbClient);
		...
		return toothpasteResult.rows[0];
	} catch (error) {
		await dbClient.query("ROLLBACK");
		console.error("Error creating toothpaste:", error);
		throw error;
	} finally {
		dbClient.release();
	}
